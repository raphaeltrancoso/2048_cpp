...
#include <stdlib.h> 
#include <string>
#include "game2048_3.hpp"
using namespace std;

game2048_3::game2048_3(int s,int p){
	size = s;
	puissance = p;
	tab = new int*[size];
...
void game2048_3::drawGame(char* message){
	move(0,0);
...
void game2048_3::addRandomNumber(){
	int** tabZero= new int*[size*size];
	int leng = 0,x,y,r,n,m;
...
void game2048_3::moveUp(){
	...
	for(int j=0;j<size;j++){
		for(int i=0;i<size-1;i++){
			if(abs(tab[i][j])==abs(tab[i+1][j]) && tab[i][j]!=0){
				add = true;
...
void game2048_3::moveLeft(){
	...
	for(int i=0;i<size;i++){
		for(int j=0;j<size-1;j++){
			if(abs(tab[i][j])==abs(tab[i][j+1]) && tab[i][j]!=0){
				add = true;
...
int game2048_3::gameEnd(){
	/** 1: WIN    2: LOSE   0:CONTINUE PLAY**/
	for(int i=0;i<size;i++){
		for(int j=0;j<size;j++){
			if(tab[i][j]==2048)
				return 1;
